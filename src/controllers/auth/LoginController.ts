...
class LoginController {
  public async Login (req: Request, res: Response) {
    ...
    try {
      ...
      if (user.rowCount === 0) return res.status(401).send({ error: 'Authentication failed', message: 'Email ou Senha incorretos' })

      bcrypt.compare(password, user.rows[0].senha, (err, result) => {
        if (err) return res.status(500).send({ error: err })

        if (!result) return res.status(401).send({ error: 'Authentication failed', message: 'Email ou Senha incorretos' })
      ...
      return res.status(500).send({ error: 'Login Failed: ' + err, message: 'Ocorreu um erro interno' })
    }
  }
}
