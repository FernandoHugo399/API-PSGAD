...
class DeleteCategoryController {
  public async DeleteCategory (req: Request, res: Response) {
    if (!req.body.id) return res.status(200).send({ error: 'All data has not been filled in. { id }', message: 'Id da categoria não foi passada' })
    try {
      const deleteResult = await db.query('delete from categoria where id_categoria = $1', [req.body.id])
